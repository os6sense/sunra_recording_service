...
# ==== Description
# Provide a webservice (json interface only) allowing for calls to start and
# stop recordings locally.
#
# +start+ will call the recording api # to launch multiple capture clients in
#         mpX formats.
# +stop+ will stop any recordings.
# +status+ will provide information about the current state of the service.
class Sunra::Recording::Service < Sinatra::Base
  ...
  def validate_api_key(api_key)
    puts api_key
    if @global_config.api_key != api_key
  ...
  get '/' do
    return { "name" => "Sunra::Recording::Service",
       "description" => "start - start recording and return the status
